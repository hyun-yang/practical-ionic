...
export class SignupPage implements OnInit {
	...
	constructor(private router: Router,
		...
		this.userForm = this.formBuilder.group({
			username: ['', Validators.compose([Validators.minLength(2), Validators.maxLength(30), Validators.required])],
	...
	onSubmit() {
		...
		if (this.userForm.controls.username.valid && this.userForm.controls.email.valid && this.userForm.controls.password.valid) {
			this.utilService.showLoading(this.translateService.instant('PROCESSING'));
			this.authenticationService.signUp(this.userForm.controls.username.value, this.userForm.controls.email.value,
